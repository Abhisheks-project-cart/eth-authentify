...
export default class SetOwner extends Component {
  state = {
    productId: "",
    retailer: "",
    customer: "",
  ...
  handleSubmit = async () => {
    const { productId, retailer, customer, success } = this.state;

    await this.props.callback(productId, retailer, customer);
    this.setState({
      success: true,
    });
  };
  ...
  render() {
    const { productId, success, retailer, customer } = this.state;
    return (
      <div>
        <div>
          <div>
